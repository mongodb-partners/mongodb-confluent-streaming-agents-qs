...
locals {
  # Supported regions (MongoDB Atlas M0 free tier supported)
  region_mapping = {
    "us-east-1"      = "us-east-1"
    "us-west-2"      = "us-west-2"
    "sa-east-1"      = "sa-east-1"
...
resource "confluent_api_key" "app-manager-flink-api-key" {
  ...
  # Match the depends_on of the Kafka and SR API keys.
  # Without this the Flink API key can be created before the SA has any
  # privilege, producing a key whose principal has no permissions → the
  # SR propagation race that terraform_runner.run_terraform retries.
  depends_on = [
    confluent_role_binding.app-manager-kafka-cluster-admin
  ]
}
...
# ------------------------------------------------------
# LLM INFRASTRUCTURE — AWS BEDROCK
# ------------------------------------------------------
...
resource "confluent_flink_connection" "bedrock_connection" {
  ...
  type              = "BEDROCK"
  endpoint          = "https://bedrock-runtime.${var.cloud_region}.amazonaws.com/model/${var.bedrock_model_id}/invoke"
...
# Text Generation Model — AWS (Bedrock)
resource "confluent_flink_statement" "llm_textgen_model" {
  ...
  # max_tokens capped at 500 (was 50000). At 50000 a
  # single response on Claude Sonnet 4.6 costs ~$0.75; 1000 surge rows
  # per minute on adversarial input = ~$45k/day. 500 tokens is
  # sufficient for a 1-2 sentence anomaly explanation per the prompt.
  statement = "CREATE MODEL `${confluent_environment.staging.display_name}`.`${confluent_kafka_cluster.standard.display_name}`.`llm_textgen_model` INPUT (prompt STRING) OUTPUT (response STRING) WITH ( 'provider' = 'bedrock', 'task' = 'text_generation', 'bedrock.connection' = '${confluent_flink_connection.bedrock_connection.display_name}', 'bedrock.params.max_tokens' = '500' );"
