...
def find_state_files(base_dir: Path) -> Tuple[Path, Path]:
    """
    ...
    Returns:
        Tuple of (core_state_path, agents_state_path)
    ...
    agents_state = provider_dir / "agents" / "terraform.tfstate"
    ...
    return core_state, agents_state
...
def extract_kafka_credentials(
    ...
    logger.info("Extracting AWS credentials from terraform state...")

    # Find state files
    core_state, agents_state = find_state_files(project_root)
    ...
    # Get outputs from agents state (optional)
    agents_outputs = {}
    if agents_state.exists():
        logger.info("Reading agents terraform state...")
        agents_outputs = run_terraform_output(agents_state)
        logger.info(f"Agents outputs available: {sorted(agents_outputs.keys())}")
    else:
        logger.warning(f"Agents state file not found (optional): {agents_state}")
    ...
    for tf_key, cred_key in required_keys.items():
        ...
        elif tf_key in agents_outputs:
            credentials[cred_key] = agents_outputs[tf_key]
        else:
    ...
    if missing_keys:
        ...
        for key in sorted(core_outputs.keys()):
            logger.error(f"  - {key}")
        if agents_outputs:
            logger.error("Available agents outputs:")
            for key in sorted(agents_outputs.keys()):
                logger.error(f"  - {key}")
...
def validate_terraform_state(project_root: Optional[Path] = None) -> bool:
    """
    ...
    """
    try:
        extract_kafka_credentials(project_root)
        return True
...
def get_project_root(strict: bool = True) -> Path:
    """
    ...
    canonical helper for all callers. Previously
    cli_output/cli_logging/health/preflight/pipeline_logger each defined
    their own `_project_root()` (script-location-only walk). Use this
    instead — `strict=False` matches the old lenient behaviour for
    callers that need a fallback rather than an exception.

    Args:
        strict: when True (default), raise FileNotFoundError if not
            found. When False, return the parent of this file as a
            fallback so caller code never crashes on filesystem oddities
            (test envs, frozen apps, etc.).
