...
def _check_flink(outputs: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not all([key, secret, org, env, ep]):
        return [_entry(name="flink", status="unknown",
                       detail="no terraform outputs")]
    cred = base64.b64encode(f"{key}:{secret}".encode()).decode()
    base = f"{ep}/sql/v1/organizations/{org}/environments/{env}/statements"
    statements = [
        "zone-traffic-sink-insert", "anomaly-detection-insert",
        "anomalies-enriched-ctas", "anomalies-enriched-insert",
        "anomalies-sink-insert", "dispatch-insert",
    ]
    results = []
    for n in statements:
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as r:
                ...
                status = "ok" if phase in ("RUNNING", "COMPLETED") else "fail"
                results.append(_entry(
                    name=n, status=status, detail=detail, phase=phase,
                ))
...
def _check_asp(creds: dict[str, str]) -> list[dict[str, Any]]:
    ...
    if not (pub and priv and proj):
        return [_entry(name="asp", status="unknown",
                       detail="missing Atlas Admin keys")]
    try:
        import requests
    ...
    try:
        ...
        for p in r.json().get("results", []):
            ...
            ckpt = (p.get("stats", {}).get("lastCheckpoint", {})
                     .get("commitTime", {}).get("$date", ""))
            ...
            if state == "STARTED":
                if ckpt:
                    ...
                    status, detail = "warn", "STARTED but no checkpoint yet (possible stall)"
            else:
...
def _check_kafka(outputs: dict[str, Any]) -> list[dict[str, Any]]:
    bootstrap = (outputs.get("confluent_kafka_cluster_bootstrap_endpoint")
                 or {}).get("value")
    ...
    if not (bootstrap and key and secret):
        return [_entry(name="kafka", status="unknown",
                       detail="no terraform outputs")]
    try:
        from confluent_kafka import Consumer, TopicPartition
    except ImportError:
        return [_entry(name="kafka", status="unknown",
                       detail="confluent_kafka not installed")]
    ...
    consumer_config = {
        ...
        "sasl.username": key, "sasl.password": secret,
        "group.id": f"health-check-readonly-{uuid4().hex[:8]}",
        "enable.auto.commit": False,
    }
    topics = [
        "ride_requests", "windowed_traffic", "zone_traffic_sink",
        "anomalies_per_zone", "anomalies_sink", "anomalies_enriched",
        "completed_actions",
    ]
    results = []
    ...
    try:
        ...
        for topic in topics:
            ...
            if tmd is None or tmd.error is not None:
                results.append(_entry(name=topic, status="unknown",
...
def _check_mongo(creds: dict[str, str]) -> list[dict[str, Any]]:
    ...
    if not conn:
        return [_entry(name="mongo", status="unknown",
    ...
    for db, coll in targets:
        try:
            ...
            results.append(_entry(
                name=f"{db}.{coll}", status="ok", count=n,
            ))
        except Exception as e:
            results.append(_entry(
                name=f"{db}.{coll}", status="unknown", detail=str(e)[:120],
            ))
    return results
...
def _check_mcp(creds: dict[str, str]) -> list[dict[str, Any]]:
    ...
    if not (url and token):
        return [_entry(name="mcp", status="unknown",
    ...
    try:
        ...
        if r.status_code == 200:
            return [_entry(name="mcp", status="ok",
                           detail="initialize returned 200")]
        # error discrimination.
        if r.status_code in (401, 403):
            return [_entry(name="mcp", status="fail",
...
def _fmt_text(report: dict[str, Any]) -> str:
    ...
    for e in report["flink"]:
        ...
        phase = e.get("phase", "")
        detail = e.get("detail", "")
        lines.append(f"  {sym}  {e.get('name'):<32} {phase:<12} {detail}")
    lines.append("")
    lines.append("ASP Processors:")
    for e in report["asp"]:
        sym = {"ok": "✓", "fail": "✗", "warn": "⚠", "unknown": "?"}.get(e.get("status"), "?")
        state = e.get("state", "")
    ...
    for e in report.get("mcp", []):
        sym = {"ok": "✓", "fail": "✗", "unknown": "?"}.get(e.get("status"), "?")
        detail = e.get("detail", "")
        lines.append(f"  {sym}  {e.get('name'):<32} {detail}")
    return "\n".join(lines)
